    ...
    body: (
      <>
        <section>
          ...
          <p>
            Use durable cloud storage or an approved document portal when the recipient needs
            reliable later access, version history, an audit trail, or an organizational record.
            Use a purpose-built, independently reviewed system when regulation or a high-risk
            threat model requires it.
          </p>
          <p>
            A disposable room fits a narrower job: two known participants are online, one needs to
            transfer a file now, and neither wants the relay to become a permanent file archive.
          </p>
        </section>
        ...
        <section>
          ...
          <ol>
            ...
            <li>
              The sender splits it into 64 KiB chunks and encrypts each chunk in the browser before
              sending it through the relay.
            </li>
          ...
          <p>
            The Cloudflare relay handles ciphertext and does not persist the file as a server-side
            attachment. The sender must remain connected because elm.chat is a live transfer, not
            an asynchronous file locker.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Destroying the room ends its server-side lifecycle and prevents elm.chat from becoming
            a durable transcript or file repository. It does not erase a file the recipient saved,
            copied, photographed, backed up, or forwarded, and it cannot clean clipboard history,
            browser downloads, notification previews, or a compromised endpoint.
          </p>
          <p>
    ...
    body: (
      <>
        <section>
          ...
          <p>
            In a distributed system, data rarely has one address. A user action can produce an
            application record, search index entry, cache value, analytics event, retry payload,
            queue message, log line, database replica, backup block, notification preview, and one
            or more client-side copies. Hiding the application record does not resolve the rest.
          </p>
          <p>
            A credible deletion promise therefore needs a scope, an authority, a deadline, and a
            failure model. Scope identifies the copies the service controls. Authority identifies
            who may trigger destruction. The deadline says when controlled copies become
            inaccessible and when they are physically reclaimed. The failure model explains what
            happens when a component is offline, a retry arrives late, or recovery restores older
            state.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Before designing an API, list every place the data can exist and why it exists there.
            Classify each location as authoritative storage, derived storage, transit, operational
            telemetry, backup, or participant-controlled state. Give every class an owner and a
            retention rule. If a team cannot enumerate a copy, it cannot make a defensible claim
            about deleting that copy.
          </p>
          <div className="marketing-table-wrap">
            <table>
              <thead>
                <tr><th>State class</th><th>Typical examples</th><th>Deletion question</th></tr>
              </thead>
              <tbody>
          ...
          <figure>
            <img
              src="/deletion-state-inventory.svg"
              alt="Six distributed data state classes: authoritative, derived, transit, operational, recovery, and participant-controlled copies"
              width={1200}
              height={675}
              loading="lazy"
            />
        ...
        <section>
          ...
          <p>
            A boolean such as <code>deleted=true</code> is usually too weak. Systems often need a
            lifecycle that distinguishes an active object from one that is closed to new writes,
            destroyed for normal access, and eventually reclaimed from recovery media. The
            transition into the destroyed state should be monotonic: delayed requests, retries, and
            replayed events must not reopen it.
          </p>
          <pre>
            <code>{`ACTIVE -> SEALED -> DESTROYED -> RECLAIMED
          |          |
          +--expire--+

Invariant: no transition leaves DESTROYED or RECLAIMED.`}</code>
          </pre>
          <p>
            Store the destruction marker wherever the system stores the object&apos;s identity, and
            make every write path check it. If a backup restore can resurrect an older active
            record, the recovery procedure must replay later destruction markers before the object
            becomes reachable.
          </p>
          <figure>
            <img
              src="/deletion-state-lifecycle.svg"
              alt="Irreversible lifecycle from Active to Sealed, Destroyed, and Reclaimed, with expiry leading to destruction"
              width={1200}
              height={675}
              loading="lazy"
            />
        ...
        <section>
          ...
          <p>
            ...
            and whether destruction has occurred. The data plane carries the content. Keeping these
            responsibilities separate makes it possible to retain the minimum state needed to
            enforce an irreversible tombstone without retaining the content that the tombstone is
            meant to retire.
          </p>
          <p>
            This separation also sharpens review. A reviewer can ask whether the control plane
            authorizes destruction correctly, whether the data plane writes content anywhere
            durable, and whether either plane leaks content into logs or metrics. Encryption helps
            with content confidentiality, but it does not answer lifecycle, metadata, or endpoint
            questions by itself.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Most deletion bugs are resurrection bugs. A mobile client reconnects with an old
            offline mutation. A queue retries a create event after a tombstone. A cache repopulates
            from a lagging replica. A restore process brings back a record but not the later delete.
            A search index remains queryable after the primary row is gone. These are ordinary
            distributed-systems behaviors, so deletion tests must exercise them deliberately.
          </p>
          <ul>
        ...
        <section>
          ...
          <p>
            A successful API response proves that one component accepted a request. It does not
            prove that the system can no longer serve, search, replay, restore, or infer the data.
            Verification should test absence across every controlled state class and should repeat
            those checks after component restarts, delayed delivery, cache refresh, replica catchup,
          ...
          <figure>
            <img
              src="/deletion-verification-loop.svg"
              alt="Deletion verification loop: create a harmless marker, destroy it, attack recovery paths, search controlled state, and record the remaining boundary"
              width={1200}
              height={675}
              loading="lazy"
            />
        ...
        <section>
          ...
          <p>
            “Gone forever” is almost never an engineering statement. A service can promise that it
            no longer serves content, that controlled content stores were purged, or that backups
            age out within a documented window. It cannot promise that a recipient forgot, that a
            screenshot vanished, that a compromised endpoint was cleaned, or that network metadata
            was never observed unless the architecture actually provides those properties.
          </p>
          <p>
            Product copy should name the actor and the scope: “the relay no longer retains the room”
            is testable; “this conversation leaves no trace” is not. Honest language is not a
            conversion penalty. It is part of the interface contract, especially when users are
            choosing a tool because they want less durable data.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            elm.chat applies a narrow version of this model. A Durable Object stores room policy,
            status, creator capability, and invite state. Connected browsers hold the current
            conversation history. The relay forwards encrypted messages and file chunks without
            persisting a server-side transcript. Destroying the room changes its control-plane state
            so later joins and writes are rejected.
          </p>
          <p>
        ...
        <section>
          ...
          <p>
            ...
            influence behavior after destruction, and what evidence proves they cannot bring the
            object back?” That question turns deletion from a user-interface gesture into a system
            property engineers can model, test, monitor, and explain.
          </p>
          <p>
            The complete elm.chat source, architecture, threat model, and deployment path are public
            under AGPL-3.0 for anyone who wants to challenge the example or adapt the design to a
    ...
    body: (
      <>
        <section>
          ...
          <p>
            A reusable secret link usually answers several questions at once: which room, who may
            enter, and which key decrypts the conversation. Forwarding that link forwards every
            capability it contains. elm.chat instead gives each concern a different value:
          </p>
          <ul>
          ...
          <pre>
            <code>{`/c/{roomId}?invite={inviteToken}#{roomSecret}`}</code>
          </pre>
          <p>
            Browsers do not send the fragment in a normal HTTP request, so the relay receives the
            room ID and invite token but not the room secret through ordinary navigation. This
            narrows the relay&apos;s authority; it does not make the connection anonymous.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            An invite is not a Boolean named <code>valid</code>. It has time and ownership semantics.
            The current elm.chat record contains a random token, creation and expiry times, and
            optional consumption, consuming-session, and revocation fields.
          </p>
          <pre>
            <code>{`type Invite = {
  token: string;
  ...
  revokedAt?: number;
          ...
          <p>
            Creation requires the creator capability. The room generates the token, applies a
            ten-minute default lifetime with a one-minute floor, persists the record, and returns
            it only after storage succeeds.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            The join transition rejects a missing, revoked, consumed, or expired invite. For the
            winning session, it writes <code>consumedAt</code> and
            <code>consumedBySessionId</code> before the first storage await. That ordering matters:
            another join handled by the same room sees the in-memory consumed state immediately,
            even while persistence is in progress.
          </p>
          <pre>
            <code>{`if (!invite || invite.revokedAt || invite.consumedAt || invite.expiresAt <= now) {
  rejectJoin();
}
...
admitSocket();`}</code>
          ...
          <p>
            Keeping this transition inside the room&apos;s single Durable Object avoids a separate
            read-then-write race across stateless workers or database replicas. If your platform
            does not provide single-owner coordination, use a conditional write or transaction
            that changes exactly one unconsumed record.
          </p>
          <p>
            Cloudflare&apos;s current <a href="https://developers.cloudflare.com/durable-objects/best-practices/rules-of-durable-objects/" rel="noreferrer" target="_blank">Durable Objects rules</a> describe each object as a globally unique, single-threaded coordination point and explain the input and output gates around storage. The practical guarantee here is <em>at most one new session admitted</em>, not magical exactly-once delivery: a failure after persistence can burn an invite without completing the join.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            At-most-once admission without reconnect semantics creates a brittle product: a page
            reload consumes a second invite or locks out the intended participant. elm.chat stores
            a random room-scoped session ID in browser session storage. The session that consumed
            the invite may reconnect with the same ID while the invite remains unrevoked; a new
            session is rejected.
          </p>
          <p>
            This is continuity, not identity verification. Anyone who obtains the invite before
            redemption can still win the race, and browser state can be copied or compromised.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Revoking an unused invite prevents a future join. Revoking a consumed invite also
            disconnects its currently connected consuming session. Otherwise a creator-facing
            “revoke” button changes a database flag while leaving the admitted participant online.
          </p>
          <p>
            Room destruction is a broader transition: it closes every socket and ends the room
            lifecycle. Neither action can erase plaintext a participant already saved, copied,
            photographed, forwarded, or backed up.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Count creation and redemption only in aggregate if you need funnel evidence. Do not put
            room IDs, invite tokens, participant IDs, or message data into growth analytics.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            The complete TypeScript path is public: the browser assembles the capability URL, the
            Worker routes requests, and the room-scoped Durable Object owns invite state and join
            ordering. The project is AGPL-3.0-or-later and includes a one-click Cloudflare deploy
            path for independent testing.
          </p>
          <ul>
            <li><a href={`${GITHUB_URL}/blob/main/apps/web/src/App.tsx`} rel="noreferrer" target="_blank">Browser invite and reconnect flow</a></li>
            <li><a href={`${GITHUB_URL}/blob/main/durable-objects/room/src/room.ts`} rel="noreferrer" target="_blank">Invite state machine and join transition</a></li>
...
function RelatedGuides({ current }: { current: MarketingSlug }) {
  const guides: Array<{ slug: MarketingSlug; label: string }> = [
    ...
    { slug: "send-a-file-securely", label: "How to send a file securely" },
    ...
    {
      slug: "deletion-distributed-systems-contract",
      label: "Deletion as a distributed-systems contract"
    },
    {
    ...
    {
      slug: "single-use-invite-links",
      label: "Build single-use invite links as capabilities"
    }
  ];
